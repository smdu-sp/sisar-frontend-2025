/** @format */

import DataTable, { TableSkeleton } from '@/components/data-table';
import { BotaoCadastroFlutuante } from '@/components/cadastro/cadastro-lista';
import { Filtros } from '@/components/filtros';
import Pagination from '@/components/pagination';
import { auth } from '@/lib/auth/auth';
import * as categorias from '@/services/categorias';
import { ICategoria, IPaginadoCategorias } from '@/types/categorias';
import { Suspense } from 'react';
import { categoriaColumns } from './_components/columns';
import ModalCategoria from './_components/modal-categoria';

export default function CategoriasSuspense({
	searchParams,
}: {
	searchParams: Promise<{ [key: string]: string | string[] | undefined }>;
}) {
	return (
		<Suspense fallback={<TableSkeleton />}>
			<CategoriasPage searchParams={searchParams} />
		</Suspense>
	);
}

async function CategoriasPage({
	searchParams,
}: {
	searchParams: Promise<{ [key: string]: string | string[] | undefined }>;
}) {
	let { pagina = 1, limite = 10, total = 0 } = await searchParams;
	const { busca = '' } = await searchParams;
	let dados: ICategoria[] = [];

	const session = await auth();
	if (session?.access_token) {
		const response = await categorias.buscarTudo(
			session.access_token,
			+pagina,
			+limite,
			busca as string,
		);
		if (response.ok && response.data) {
			const paginado = response.data as IPaginadoCategorias;
			pagina = paginado.pagina || 1;
			limite = paginado.limite || 10;
			total = paginado.total || 0;
			dados = paginado.data || [];
		}
	}

	return (
		<div className='w-full px-0 md:px-8 relative pb-20 md:pb-14 h-full md:container mx-auto'>
			<h1 className='text-xl md:text-4xl font-bold'>Categorias</h1>
			<div className='grid grid-cols-1 max-w-sm mx-auto md:max-w-full gap-y-3 my-5 w-full'>
				<Filtros
					camposFiltraveis={[
						{ nome: 'Busca', tag: 'busca', tipo: 0, placeholder: 'Buscar categoria...' },
					]}
				/>
				<DataTable columns={categoriaColumns} data={dados} />
				{dados.length > 0 && (
					<Pagination total={+total} pagina={+pagina} limite={+limite} />
				)}
			</div>
			<BotaoCadastroFlutuante>
				<ModalCategoria />
			</BotaoCadastroFlutuante>
		</div>
	);
}
