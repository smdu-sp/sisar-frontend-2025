...
import ModalNovoProcesso from '../processos/_components/modal-novo-processo';
...
export default function AnaliseSuspense({
	...
	return (
		<Suspense fallback={<TableSkeleton />}>
			<AnalisePage searchParams={searchParams} />
		</Suspense>
	);
}

async function AnalisePage({
	searchParams,
}: {
	searchParams: Promise<{ [key: string]: string | string[] | undefined }>;
}) {
	...
	if (session?.access_token) {
		const response = await analise.buscarTudo(
	...
	return (
		<div className='px-0 md:px-8 relative pb-20 md:pb-14 h-full container mx-auto'>
			<h1 className='text-xl md:text-4xl font-bold'>Em Análise</h1>
